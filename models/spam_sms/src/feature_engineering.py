...
def add_text_features(df: pd.DataFrame) -> pd.DataFrame:
    app_logging.info("Adding text-based features")
...
def split_data(df: pd.DataFrame, test_size: float, random_state: int):
    ...
    app_logging.info("Data split into train/test")
...
def tokenize_and_pad(X_train, X_test, vocab_size=10000, max_len=100):
    app_logging.info("Tokenizing and padding text data")
    ...
    app_logging.info("Tokenization and padding completed")
    return tokenizer, X_train_pad, X_test_pad

# ----------------------------
# Save tokenizer to artifacts
# ----------------------------
def save_tokenizer(tokenizer, artifacts_dir: Path, filename: str = "tokenizer.json"):
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    # Save tokenizer as JSON (Keras native)
    path_json = artifacts_dir / filename
    with open(path_json, "w", encoding="utf-8") as f:
        f.write(tokenizer.to_json())

    # Optional: save pickle for backward compatibility
    path_pkl = artifacts_dir / "tokenizer.pkl"
    with open(path_pkl, "wb") as f:
        pickle.dump(tokenizer, f)

    app_logging.info(f"Tokenizer saved at {path_json} and {path_pkl}")
...
if __name__ == "__main__":
    try:
        ...
        if df.empty:
            app_logging.warning("No data found in processed directory. Exiting.")
        else:
            # Feature engineering
            ...
            # Tokenize + pad
            tokenizer, X_train_pad, X_test_pad = tokenize_and_pad(
                X_train, X_test, vocab_size=vocab_size, max_len=max_len
            )
            ...
            app_logging.info("Feature engineering completed successfully")

    except Exception as e:
        app_logging.exception(f"Feature engineering failed: {e}")
        raise
